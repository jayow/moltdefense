#!/usr/bin/env node
/**
 * Moltdefense Defender Test Agent
 *
 * Usage:
 *   node defender.js [options]
 *
 * Options:
 *   --strategy <name>  Strategy to use (default: balanced)
 *   --name <name>      Agent name (default: defender_<random>)
 *   --server <url>     Server URL (default: http://localhost:3000)
 *   --quiet            Minimal output
 *   --help             Show help
 */

const { generateDefenseBuild, calculateDefenseCost, DEFEND_STRATEGIES, getRandomStrategy, getAdaptiveDefenseStrategy } = require('./strategies');

// Parse command line arguments
const args = process.argv.slice(2);
const options = {
  strategy: 'balanced',
  name: `defender_${Math.random().toString(36).slice(2, 8)}`,
...
for (let i = 0; i < args.length; i++) {
  switch (args[i]) {
    ...
    case '-h':
      console.log(`
Moltdefense Defender Agent

Usage: node defender.js [options]
...
Available strategies: ${DEFEND_STRATEGIES.join(', ')}
      `);
      process.exit(0);
  }
}
...
async function main() {
  ...
  if (options.adaptive) {
    ...
    options.strategy = await getAdaptiveDefenseStrategy(options.server);
  }
  ...
  // Generate build
  const build = generateDefenseBuild(options.strategy);
  const cost = calculateDefenseCost(build);
...
function printResults(results) {
  const won = results.winner === 'defender';
  ...
  if (won) {
    ...
    log('Attacker wins. Better luck next time!');
  }
